...
sudo apt-get install dnsutils fail2ban samba iotop htop iftop ntop nethogs pv lzop xz-utils python python-dev libjpeg-dev libfreetype6-dev python-setuptools python-pip x11-apps x11-xserver-utils xterm vim vim-gtk pyroom git conky-std alsa-utils mpg321 lame fonts-inconsolata screen screenie build-essential bluez dos2unix libgnutls28-dev telnet
...
tar -xzf csf.tgz
cd csf  || exit
# install.sh is for cPanel-only servers. NEVER install it on non-cPanel servers.
# Instead, run install.generic.sh, which is designed to run in either a cli
# environment or a Webmin-enabled environment.
